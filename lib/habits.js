...
function optimizePlan(input = {}) {
  ...
  const energyOps = Array.isArray(input.energyOperations) ? input.energyOperations.slice() : [];
  if (!energyOps.length && Number.isFinite(Number(input.watts)) && input.pvSurplusLoadStateId) {
  ...
  const filteredEnergy = energyOps.filter((op) => !cooldownIds.has(op.id));
  const operations = mergeOperations([habitOps, filteredEnergy]);
