...
class BridgeRuntime {
    ...
    for (const op of operations) {
      ...
      try {
        ...
        checks.push({
          ...
          error: blockedByGuard ? toStructuredError({ code: guard.code, message: guard.message, details: guard }) : undefined,
        });
      } catch (err) {
        checks.push({
          ...base,
      ...
      if (options.skipGuards !== true) {
        ...
        if (!guard.ok) {
          ...
          results.push({
            ok: false,
            id: op.id,
            error: toStructuredError({ code: guard.code, message: guard.message, details: guard }),
          });
          delete pending[op.id];
          continue;
        }
  ...
  async executeAction(payload) {
    ...
    if (action === 'help') {
      return {
        ...
        quickStart: [
          ...
          { action: 'checkGuards', payload: { action: 'checkGuards', id: '0_userdata.0.pump.well', value: true } },
        ],
      };
    }
    ...
    if (action === 'checkGuards') {
      ...
      return this.validatePlan(operations, { confirmation: payload.confirmation === true });
    }
